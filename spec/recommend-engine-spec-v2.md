...
1. **関連性ベース**: 複数ソース（KKBOX, Last.fm, MusicBrainz, YouTube Music）から候補を並列収集
2. **雰囲気ベース**: Deezer BPM/Gain + MusicBrainz タグによる類似度でランキング
3. **ジャンル考慮**: MusicBrainz タグ + Spotify アーティストジャンルでフィルタリング・ボーナス/ペナルティ適用

### 候補収集ソース

| ソース        | API                          | 収集内容                         | 候補数 |
| ------------- | ---------------------------- | -------------------------------- | ------ |
| KKBOX         | GetRecommendedTracks         | シードトラックに基づくレコメンド | 30 件  |
| Last.fm       | track.getSimilar             | シードトラックの類似曲           | 30 件  |
| MusicBrainz   | Browse recordings by artist  | 同一アーティストの他の曲         | 20 件  |
| YouTube Music | get_watch_playlist (sidecar) | ラジオ/類似曲プレイリスト        | 25 件  |
...
│  │                     候補収集（並列実行）                               │  │
│  │  ┌──────────┐  ┌──────────┐  ┌─────────────┐  ┌────────────────────┐ │  │
│  │  │  KKBOX   │  │ Last.fm  │  │ MusicBrainz │  │  YouTube Music     │ │  │
│  │  │ Recommend│  │ Similar  │  │ Artist      │  │  (Python sidecar)  │ │  │
│  │  │  30件    │  │  30件    │  │ Recordings  │  │  25件              │ │  │
│  │  │          │  │          │  │  20件       │  │                    │ │  │
│  │  └────┬─────┘  └────┬─────┘  └──────┬──────┘  └─────────┬──────────┘ │  │
│  │       └──────────┬──┴───────────────┴──────────────────┘             │  │
│  │                  ↓                                                    │  │
...
│  │              特徴量取得（並列実行）                                    │  │
│  │  ┌──────────────────────┐    ┌─────────────────────────────────────┐ │  │
│  │  │   Deezer API         │    │    MusicBrainz API                  │ │  │
│  │  │ - BPM                │    │ - Tags (genre, mood, style)         │ │  │
│  │  │ - Duration (秒)       │    │ - Artist relations                  │ │  │
│  │  │ - Gain (dB)          │    │ - Artist MBID                       │ │  │
│  │  └──────────────────────┘    └─────────────────────────────────────┘ │  │
│  └──────────────────────│───────────────────────────────────────────────┘  │
│                         ↓                                                  │
│  ┌──────────────────────────────────────────────────────────────────────┐  │
│  │            Similarity Calculator V2                                  │  │
│  │  ┌────────────────┐  ┌────────────────────────────────┐              │  │
...
│    ├── MusicBrainz: ISRC検索 → タグ / アーティスト関連 / MBID    │
│    └── Spotify: アーティストのジャンル情報                        │
└─────────────────────────────────────────────────────────────────┘
                              ↓
┌─────────────────────────────────────────────────────────────────┐
│ 3. 候補トラック収集（並列実行 - マルチソース）                      │
│    ├── KKBOX: SearchByISRC → GetRecommendedTracks (30件)        │
│    ├── Last.fm: track.getSimilar (30件)                         │
│    ├── MusicBrainz: アーティストMBID → 他の曲 (20件)             │
│    └── YouTube Music: Search → get_watch_playlist (25件)        │
│        ※ YouTube Music は Python sidecar 経由                   │
└─────────────────────────────────────────────────────────────────┘
                              ↓
┌─────────────────────────────────────────────────────────────────┐
│ 4. 候補の重複除去・フィルタリング                                   │
│    ├── 入力トラックと同一の曲を除外                                 │
│    ├── ISRC または name+artist で重複を除去                       │
│    ├── ジャンルフィルタリング（関連ジャンルのみ残す）               │
│    └── 候補を最大50件に制限                                       │
└─────────────────────────────────────────────────────────────────┘
                              ↓
┌─────────────────────────────────────────────────────────────────┐
│ 5. 候補の Spotify 情報取得（並列実行）                             │
│    ├── ISRC がある候補: Spotify ISRC 検索                        │
│    └── ISRC がない候補: Spotify 名前検索 (artist + track)        │
│        ※ Last.fm/YouTube Music の候補は名前検索                  │
└─────────────────────────────────────────────────────────────────┘
                              ↓
┌─────────────────────────────────────────────────────────────────┐
│ 6. 候補の特徴量取得（並列実行）                                     │
...
Last.fm の track.getSimilar API を使用して類似曲を取得します。

#### エンドポイント

```bash
GET https://ws.audioscrobbler.com/2.0/?method=track.getsimilar
    &artist={artist_name}
    &track={track_name}
    &api_key={api_key}
    &limit={limit}
...
| `artist.name` | アーティスト名                 |
| `match`       | 類似度スコア (0-1)             |
| `mbid`        | MusicBrainz Track ID（あれば） |

#### 注意事項

- **認証**: API Key 必須（https://www.last.fm/api/account/create で無料取得）
- **レート制限**: 特に厳しくないが、1 秒に数リクエスト程度が推奨
- **ISRC なし**: artist + track name のみ返却されるため、Spotify で名前検索が必要

---

### YouTube Music API (ytmusicapi sidecar)

YouTube Music の類似曲を取得するため、Python の ytmusicapi ライブラリを使用した sidecar サービスを提供します。

#### アーキテクチャ

```
┌──────────────────┐      HTTP       ┌───────────────────────────┐
│ Go メインサーバー │ ─────────────►  │ Python sidecar (FastAPI) │
│ (tracktaste)     │                 │ port 8081                │
│                  │  /similar/{id}  │                          │
│                  │  /search?q=     │  ytmusicapi              │
└──────────────────┘                 └───────────────────────────┘
```
...
| Path                  | Method | 説明                         |
| --------------------- | ------ | ---------------------------- |
| `/similar/{video_id}` | GET    | video ID から類似曲を取得    |
| `/search?q={query}`   | GET    | 曲を検索して video ID を取得 |
| `/health`             | GET    | ヘルスチェック               |
...
- **認証不要**: ytmusicapi は公開データのみアクセス
- **非公式 API**: YouTube の仕様変更で動作しなくなる可能性あり
- **ISRC なし**: artist + track name のみ返却されるため、Spotify で名前検索が必要
