...
BearerToken は 2 層キャッシュで管理しています。

| 層  | 種別       | 特徴                                 |
| --- | ---------- | ------------------------------------ |
| L1  | インメモリ | 高速、揮発性（サーバー再起動で消失） |
| L2  | Redis      | 永続化、複数インスタンス間で共有可能 |
...
トークン取得時:
  L1(メモリ)にあり → 即返却
       ↓ なし
  L2(Redis)にあり → L1に昇格 → 返却
       ↓ なし
  APIから取得 → L1とL2に同時保存 → 返却

トークン保存時:
  L1(メモリ)に保存（必須）
  L2(Redis)に保存（Redisが利用可能な場合）
```

Redis が利用不可の場合でも、L1（インメモリ）キャッシュのみで動作します。

#### 自動トークンリフレッシュ

外部 API（Spotify/KKBOX）が認証エラー（401 Unauthorized または 400 Bad Request）を返した場合、自動的にトークンを再取得してリトライします。

```
APIリクエスト
    ↓
レスポンス 401/400?
    Yes → キャッシュからトークン削除 → 新しいトークン取得 → 1回のみリトライ
    No  → 通常処理続行
```

この機能により、以下のケースを自動的にリカバリーします：

- Redis に残った期限切れトークン
- API 側でトークンが無効化された場合
- サーバー再起動後に Redis から復元した古いトークン
